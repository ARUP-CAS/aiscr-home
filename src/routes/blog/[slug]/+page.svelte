<script lang="ts">
	...
	// Parse multiple authors (comma-separated)
	function parseAuthors(post: typeof data.post) {
		const names = post.author.split(',').map((s: string) => s.trim());
		const roles = post.authorRole.split(',').map((s: string) => s.trim());
		const images = post.authorImage.split(',').map((s: string) => s.trim());
	...
	const authors = parseAuthors(data.post);
</script>
