<script lang="ts">
	...
	import { fetchNewsItem, localeFromPathname, type FeedItem } from '$lib/feed';

	let { data }: { data: PageData } = $props();

	// Prerenderovaný obsah se v prohlížeči obnoví z živého feedu,
	// takže úpravy článku se propíší bez rebuildu webu.
	let liveItem = $state<FeedItem | null>(null);
	const post = $derived(liveItem?.slug === data.post.slug ? liveItem : data.post);

	$effect(() => {
		const slug = data.post.slug;
		const feedLocale = localeFromPathname(page.url.pathname);
		fetchNewsItem(fetch, feedLocale, slug).then((item) => {
			if (item) liveItem = item;
		});
	});
	...
	const authors = $derived(post.authors ?? []);
	// Obrázky z feedu jsou absolutní URL; bez úvodního obrázku se použije placeholder
	const coverImage = $derived(post.image ?? `${base}/images/blog/placeholder.webp`);

	// HTML sanitizuje už generátor feedu; druhá vrstva na klientovi kryje
	// případ kompromitovaného feedu (DOMPurify potřebuje DOM, při prerenderu
	// se použije obsah z buildu tak, jak je).
	const safeHtml = $derived(browser ? DOMPurify.sanitize(post.html) : post.html);
</script>

<svelte:head>
	<title>{post.title} - AIS CR Blog</title>
	{#if post.excerpt}
	...
	<div class="w-full px-4 sm:px-6 lg:px-8" style="max-width: 1312px; margin: 0 auto;">
		<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
			...
			<div class="lg:col-span-1">
				<!-- Tlačítko zpět -->
				<a href={blogUrl} data-sveltekit-reload class="inline-flex items-center text-black hover:text-gray-700 mb-4 md:mb-6" style="font-family: 'Roboto', sans-serif; font-size: 14px; md:font-size: 16px;">
				...
				<div class="flex items-center gap-3 mb-4 md:mb-6">
					...
					{#if post.readingTime}
						<span style="font-family: 'Roboto', sans-serif; font-size: 12px; md:font-size: 14px; color: #666;">
							{m['blog.readTime']({ time: post.readingTime })}
						</span>
					{/if}
				</div>
				...
				<!-- Datum publikace -->
				<time datetime={post.date} class="text-sm md:text-base" style="font-family: 'Roboto', sans-serif; color: #666;">
					{m['blog.published']()} {formatDate(post.date)}
				</time>
			</div>
		...
		<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
			...
			<div class="lg:col-span-1 order-2 lg:order-1">
				...
				<div>
					...
					<div class="flex gap-3 md:gap-4">
						...
						<a href="https://bsky.app/intent/compose?text={encodeURIComponent(post.title + ' ' + (typeof window !== 'undefined' ? window.location.href : ''))}" target="_blank" rel="noopener noreferrer" class="text-black hover:text-gray-600 transition-colors" aria-label="Sdílet na BlueSky">
			...
			<div class="lg:col-span-2 order-1 lg:order-2">
				<article class="prose prose-sm sm:prose-base md:prose-lg max-w-none" style="font-family: 'Roboto', sans-serif;">
					<div class="prose-headings:font-['Roboto_Slab'] prose-headings:text-black prose-a:text-blue-600 prose-a:no-underline prose-a:hover:underline prose-p:text-black prose-p:leading-relaxed">
						{@html safeHtml}
					</div>
				</article>
			</div>
